...
```
config/          — @ConfigurationProperties records (AppProperties, ImageProperties, SchulfotosProperties, PicPeakProperties)
...
Properties are organized under four prefixes:

| Prefix | Record | Purpose |
|--------|--------|---------|
| `app.*` | `AppProperties` | Event code/name |
| `app.image.*` | `ImageProperties` | Watermark (opacity 0.5, scale 0.6), resize max 1200px, JPEG quality 0.9, filename postfix, logo timeouts |
...
- **Event folder** — `schulfotos/<event-name>-<event-code>/` with `klassenfoto/` and `portrait-1/` through `portrait-N/`.
- **Watermarked suffix** — `-watermarked` appended to folder names (configurable via `app.schulfotos.watermarked-suffix`).
- **Filename strip postfix** — Removes a configurable postfix (default `_NEU`) from filenames during watermarking (e.g., `MEL_6175_NEU.jpg` → `MEL_6175.jpg`). Configured via `app.image.filename-strip-postfix`.
- **PicPeak** — Optional cloud gallery platform. Each gallery gets all class photos + its own portrait folder.
- **CSV upload** — After CSV generation, the file is uploaded to `{base-url}/upload.php` via multipart POST with credentials as form fields. Requires PicPeak username/password to be configured.
...
1. Every change, including small one-line edits, must be committed with a proper Conventional Commit message.
2. Do not leave "WIP", "cleanup", "misc", or similarly vague commit titles.
3. Group related work into coherent commits; avoid mixing unrelated concerns in one commit.
4. Prefer one clear commit per logical change set; if multiple areas are touched, split by purpose (`fix`, `docs`, `test`, etc.).
5. Before finalizing a commit, run formatting and relevant tests so each commit is buildable.
...
4. Verify with `./mvnw clean verify` — all tests must pass.
