	...
	static Element render(PhotoGalleryWizardViewModel viewModel) {
		...
		if (viewModel.executionInProgress()) {
			...
			return panel("Generierung läuft", column(
					...
					text(" ").length(1),
					gauge(progress).gaugeColor(accent)
						.gaugeStyle(
		...
		if (viewModel.executionResult() != null) {
			...
			items.add(PhotoGalleryWizardUi.previewCard("Erstellte Dateien",
					List.of(PhotoGalleryWizardUi.previewLine("Klassen-Name", viewModel.executionResult().eventName(),
							...
							PhotoGalleryWizardUi.previewLine("Blätter (duplex)",
									Integer.toString(viewModel.executionResult().sheetCount()), TEXT_PRIMARY),
			...
			items.add(panel("Weiter",
					text("Enter drücken, um mit der Ordnererstellung für die Foto-Ablage fortzufahren.")
						.fg(readableText(TEXT_PRIMARY, SURFACE)))
				.rounded()
				.padding(1)
				.bg(SURFACE)
