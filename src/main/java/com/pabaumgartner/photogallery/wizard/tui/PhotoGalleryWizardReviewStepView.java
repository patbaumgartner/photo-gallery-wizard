	...
	static Element render(PhotoGalleryWizardViewModel viewModel) {
		return panel(viewModel.currentStepTitle(), column(
				...
				PhotoGalleryWizardUi
					.previewCard("Konfigurationsübersicht", List.of(
							PhotoGalleryWizardUi.previewLine("Klassen-Name",
									PhotoGalleryWizardUi.blankFallback(viewModel.requestPreview().eventName(),
											"(leer)"),
									TEXT_PRIMARY),
							PhotoGalleryWizardUi.previewLine("Event-Code", viewModel.requestPreview().eventCode(),
									TEXT_PRIMARY),
							...
							PhotoGalleryWizardUi.previewLine("QR Size", viewModel.requestPreview().qrSize() + " px",
									TEXT_PRIMARY),
							PhotoGalleryWizardUi.previewLine("Schnittlinien",
									PhotoGalleryWizardUi.booleanLabel(viewModel.requestPreview().showCuttingLines()),
									TEXT_PRIMARY),
							PhotoGalleryWizardUi.previewLine("PicPeak",
									PhotoGalleryWizardUi.booleanLabel(
											viewModel.requestPreview().picPeakEnabled()),
									TEXT_PRIMARY)),
							CYAN_NEON),
				text(" ").length(1),
				panel("Ausführen",
