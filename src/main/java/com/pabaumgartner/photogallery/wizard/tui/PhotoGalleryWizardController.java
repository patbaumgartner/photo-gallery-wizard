...
final class PhotoGalleryWizardController {
	...
	private EventResult handleArrowSelection(KeyCode keyCode, Actions actions) {
		...
		if ((state.activeStep() == PhotoGalleryWizardStep.WATERMARK
				|| state.activeStep() == PhotoGalleryWizardStep.UPLOAD) && actions.eventFolderCount() > 0) {
			if (keyCode == KeyCode.UP) {
			...
			actions.syncSelectedEventFolder();
			return EventResult.HANDLED;
		}
		return EventResult.UNHANDLED;
	}
